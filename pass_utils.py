...
def is_final_third_pass(passer_team, receiver_team, passer_x, receiver_x):
    ...
    return (
        (
            ...
            (passer_team == 0 or receiver_team == 0) # at least one member is attacking right
        ) or 
        (
            (passer_x >= 0.33 and receiver_x < 0.33) # pass into "left" final third
            (passer_team == 1 or receiver_team == 1) # at least one member is attacking left
        )
    )
...
def is_penalty_area_pass(passer_team, receiver_team, passer_x, passer_y, receiver_x, receiver_y):
    ...
    into_left_penalty_area = (
        ...
        (passer_team == 1 or receiver_team == 1) # at least one member is attacking to the left
    )
    into_right_penalty_area = (
        ...
        (passer_team == 0 or receiver_team == 0) # at least one member is attacking to the right
    )
